...
var WAAClock = module.exports = function(context, opts) {
  ...
  this._bufferSizeTolerance = context.sampleRate / this.bufferSize * 1.5;
  this._events = [];
  this._started = false;
};
...
_.extend(WAAClock.prototype, {
  ...
  // Always schedule early, never late
    play: function(func, time, repeat) {
        var ev = this._createEvent(func, time).repeat(repeat).tolerance(this.bufferSizeTolerance, 0);
        //We tick to handle immediate events
        if (this._relTime(time) <= this.bufferSizeTolerance) {
            this._tick();
        }
        return ev;
