...
var WAAClock = module.exports = function(context, opts) {
  ...
  this._bufferSizeTolerance = this.bufferSize / context.sampleRate * 1.5;
  this._events = [];
  this._started = false;
};
...
_.extend(WAAClock.prototype, {
  ...
  // Schedule `func` to run at `time`, repeat`repeat`.
  // Always schedule early, never late
    play: function(func, time, repeat) {
        console.log(this._bufferSizeTolerance);
        var ev = this._createEvent(func, time).repeat(repeat).tolerance(0, this._bufferSizeTolerance);
        //We tick to handle immediate events
        if (this._relTime(time) <= this._bufferSizeTolerance) {
            this._tick();
        }
        return ev;
  ...
  start: function() {
    if (this._started === false) {
      ...
      this._clockNode = this.context.createScriptProcessor(this.bufferSize, 1, 1);
  ...
  _handleEvent: function(event) {
    if (this.context.currentTime < event._expireTime) {
      ...
      setTimeout(function() { console.log(event) }, 0);
      event.emit('executed');
    } else {
      event.emit('expired');
